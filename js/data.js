...
class DataManager {
     ...
     * 添加新批次数据（叠加模式）
     */
    addBatch(data, fileName) {
        const batchId = ++this.batchIdCounter;
        const batchData = data.map((item, index) => ({
            ...item,
            id: `${batchId}_${index}`,
            batchId: batchId,
            batchName: fileName || `批次${batchId}`,
            // 确保经纬度是数字
            longitude: parseFloat(item['经度'] || item.longitude || item.lng || 0),
            latitude: parseFloat(item['纬度'] || item.latitude || item.lat || 0)
        })).filter(item => {
            // 过滤掉无效坐标
    ...
    removeBatch(batchId) {
        ...
        // 获取该批次的数据，用于后续清除importedKeys
        const batchData = batch.data || [];

        // 从原始数据中移除该批次的数据
        this.rawData = this.rawData.filter(item => item.batchId !== batchId);
    ...
    updateFilterOptions() {
        ...
        if (batchSelect) {
            ...
            batchSelect.innerHTML = batches.map(b =>
                `<option value="${b.id}">${b.name} (${b.count}条)</option>`
            ).join('');
